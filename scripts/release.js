  ...
  if (release.status === 200) {
    ...
    release = await gh('POST', `/repos/${OWNER}/${REPO}/releases`, JSON.stringify({
      ...
      body: `## ${TAG} — UI-апгрейд + багфиксы + удалённый сервер\n\n` +
            ...
            `### 🐛 Исправления\n` +
            `- Долгая загрузка онбординга: сокращена с 5с до 1.5с, проверка обновлений в фоне\n` +
            `- Анимация смены темы больше не перекрывает карточки выбора во время онбординга\n` +
            `- Переходы между шагами замедлены для более комфортного UX\n` +
            `- Proxy bypass для систем с глобальным прокси (Clash / V2RayN)\n\n` +
            `### 📦 Установка\n` +
