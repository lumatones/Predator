...
(async () => {
  ...
  if (release.status === 200) {
    ...
    release = await gh('POST', `/repos/${OWNER}/${REPO}/releases`, JSON.stringify({
      tag_name: TAG,
      name: TAG,
      body: `## ${TAG} — Рефакторинг и улучшения\n\n` +
            `- ScanContext класс для изоляции состояния между сканированиями\n` +
            `- parsePsJson<T>() — безопасный парсинг PowerShell JSON\n` +
            `- CFG — единый источник системных путей\n` +
            `- Константы вынесены в constants.ts\n` +
            `- Cloud-синхронизация вынесена в cloud-sync.ts\n` +
            `- Типизация: убраны any, добавлены ScanOptions\n\n` +
            `### 📦 Установка\n` +
