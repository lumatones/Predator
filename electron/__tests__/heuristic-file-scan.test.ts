...
describe('heuristicFileScan', () => {
  ...
  it('returns null for non-existent file', async () => {
    expect(await heuristicFileScan(path.join(TMP_DIR, 'ghost.exe'))).toBeNull()
  })
  ...
  it('returns null for a directory', async () => {
    ...
    expect(await heuristicFileScan(dirPath)).toBeNull()
  })

  // ── 3. Oversized file (> 100 MB) ──

  it('returns null for an oversized file (>100 MB)', async () => {
    const filepath = path.join(TMP_DIR, 'huge.bin')
    const fd = fs.openSync(filepath, 'w')
    fs.ftruncateSync(fd, 101 * 1024 * 1024) // sparse file, 101 MB
    fs.closeSync(fd)
    try {
      expect(await heuristicFileScan(filepath)).toBeNull()
    } finally {
  ...
  it('returns null for a .png image (skippable extension)', async () => {
  ...
  it('returns null for a .woff2 font file (skippable)', async () => {
  ...
  it('returns null for a known Electron DLL (d3dcompiler_47.dll)', async () => {
  ...
  it('returns null for a clean .txt file with benign content', async () => {
  ...
  it('returns low-risk result for a clean .js file (no keywords, but .js is suspicious)', async () => {
    const filepath = recent('bundle.js', 'const x = 42; console.log("hello world");')
    const result = await heuristicFileScan(filepath)
  ...
  it('returns high-risk result for a .js file with cheat keywords (aimbot + inject)', async () => {
  ...
  it('returns medium-risk result for .lua file with nightfall injection script', async () => {
  ...
  it('returns high-risk for a file named injector.exe (category name match)', async () => {
  ...
  it('adds protected path risk for a file in the FiveM mods directory', async () => {
  ...
  it('flags a masquerading svchost.exe found outside of System32', async () => {
  ...
  it('returns high risk for a .ps1 script with dma/fpga/pcileech keywords', async () => {
  ...
  it('returns non-null result for an empty .asi file (suspicious ext but no content)', async () => {
  ...
  it('returns result for a temp .exe with benign content (no known keywords)', async () => {
  ...
  it('detects fpga-related keywords in a .cfg configuration file', async () => {
  ...
  it('flags a file named eac-bypass.exe as high risk', async () => {
  ...
  it('returns shadowRuleHits for files with shadow-category matches', async () => {
