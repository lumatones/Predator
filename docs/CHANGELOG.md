...
## v0.5.1 — Онбординг: язык/токен persistence, убран демо-скан, фиксы риск-скоринга (2026-08-01)

### 🇬🇧 Выбор языка
- Язык интерфейса сохраняется в конфиг (`predator-config.json`) и восстанавливается при старте — раньше при каждом запуске был только русский
- В панель «Настройки» добавлен переключатель языка (🇷🇺/🇬🇧) — раньше выбора языка после онбординга не было вообще
- Тема оформления тоже сохраняется и восстанавливается (раньше сбрасывалась на Predator Red)

### 🔑 Токен / активация
- `tokenId` сохраняется в конфиг после активации и восстанавливается при старте — токен одноразовый на сервере, раньше повторный онбординг выдавал «Token already used»
- При завершённом онбординге приложение переходит сразу в главное меню, минуя приветственный экран (gate на загрузку конфига, без «мигания»)

### 🎯 Убран демо-скан
- Удалён шаг `onboarding-demo` (фейковое сканирование) из онбординга — теперь: Приветствие → Язык → Тема → Токен → главное меню
- Удалены `DemoScanStep.tsx`, `DEMO_FINDINGS`, демо-переводы

### 🧮 Риск-скоринг
- `classifySignal`: word-boundary матчинг коротких ключей (`esp`, `apc`, `atom`, `port`, `c2`, `proxy`, `vpn`, `dns`...) — убраны ложные срабатывания на подстроки (`response` → не `esp`, `support` → не `port`)
- Категорийные оценки: средний вес сигнала вместо двойного применения суммы весов — одна сильная улика больше не проигрывает двадцати слабым
- `rescoreResults`: `riskScore` согласован с меткой риска (critical ≥ 85), без дублирования маркеров эскалации при повторном рескаре
- Адаптивный порог вынесен в `adaptiveThreshold()`

### 🎨 UI-полировка
- `CompactScanOverlay`: эмодзи-иконки режимов → SVG, inline-стили → CSS-классы
- `FileDetailModal`: кнопка копирования SHA256, пустое состояние совпадений, цветовая схема критического риска
- `UpdateModal`: чистка inline-стилей

### 🧪 Тесты
- 6 новых тестов риск-скоринга (substring false positives, слабые vs сильные сигналы, консистентность riskScore, адаптивный порог, эскалация)
- typecheck всех проектов чистый, lint 0 ошибок, тесты 4/4 renderer
