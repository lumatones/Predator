...
2. Запусти NSIS-установщик (требуются права администратора) — это полноценная установка, а не portable-версия
3. Пройди онбординг: выбери язык, тему и авторизуйся токеном доступа
4. Выбери режим сканирования и запусти проверку

**Автообновление** - установленное приложение проверяет обновления при запуске и каждые 5 минут (дифференциальные обновления через `.blockmap`). Новые версии загружаются и устанавливаются прямо из приложения — скачивать `.exe` вручную не нужно.

> ⚠️ **Для пользователей старых portable-версий:** portable `.exe` не может обновиться сам по себе. Скачай и установи новый NSIS-установщик вручную один раз — после этого автообновление заработает автоматически.
